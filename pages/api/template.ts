...
export default async function handler(
  ...
  const templateWithLanguage = {
    ...bodyTemplate,
    properties: { language: languageChoice, ...bodyTemplate.properties },
    required: ["language", ...bodyTemplate.required],
  };
  const requestTemplate: FlatfileTemplate = options.language
  ...
  if (foundEmbeds.length > 0) {
    embed = await updateEmbed(flatfileAccessToken, foundEmbeds[0].id, [
      template.id,
    ]);
  } else {
